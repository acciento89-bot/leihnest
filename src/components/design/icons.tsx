...
export type NestIconName = "lock" | "eye" | "eyeOff" | "home" | "box" | "calendar" | "people" | "leaf" | "arrow" | "plus" | "search" | "settings" | "check" | "shield" | "back" | "chevron" | "chart" | "activity" | "mail" | "school" | "church" | "theatre" | "play" | "return" | "clock";
export function NestIcon({ name, className = "" }: { name: NestIconName; className?: string }) {
  const icons: Record<NestIconName, ReactNode> = {
  ...
  return <svg className={`nest-icon ${className}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{icons[name]}</svg>;
}
