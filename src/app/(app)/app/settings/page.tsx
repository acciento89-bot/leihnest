...
import { canManageBilling, canManageGroupImage } from "@/features/groups/permissions";
...
export default async function SettingsPage({searchParams}:{searchParams:Promise<{billing?:string}>}){
  ...
  const [groupMedia,subscription]=membership?await Promise.all([
    db.mediaAsset.findFirst({where:{groupId:membership.groupId,kind:"GROUP"},orderBy:{createdAt:"desc"}}),
  ...
  return <section><PageHeading title={t.settings} description={t.settingsHint}/><div className="ws-settings-grid">
    ...
    {membership&&<BillingControls locale={locale} canManage={canManageBilling(membership.role)} snapshot={billingSnapshot} billingState={params.billing}/>}
