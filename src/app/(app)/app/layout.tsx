...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const memberCount=membership ? await db.membership.count({where:{groupId:membership.groupId}}) : 0;
  return <div className="workspace concept-workspace" lang={locale}>
    ...
    <div className="ws-main"><div className="ws-topbar"><div className="ws-person"><span className="nest-group-picture" aria-hidden="true"/><div><strong>{membership?.group.name ?? t.welcome}</strong><small>{membership ? `${memberCount} ${t.members} · ` : ""}{locale==="de"?"Gemeinsam mehr möglich":"More possible together"}</small></div></div>
